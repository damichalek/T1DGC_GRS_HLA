...
raincloudPlot = function(data, path, filename, ancestry, ext){
  data %>% 
    ...
    scale_fill_manual(values = c("#FDDDA0", "#74A089")) +
    scale_color_manual(values = c("#FDDDA0", "#74A089")) +
    theme_bw() +
    xlab("") +
    ylab(bquote(bold(T1D~GRS)[bold(HLA-SNP-.(ancestry))])) + # Dynamic Subscript with bquote()
...
mapply(raincloudPlot, dataList, "HLA/Graphs/GRS/", "grs_raincloud_plot_snp", ancestries, ".png")
